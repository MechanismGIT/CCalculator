...
int main() {
    const char* expr = "3 + 4 * (2 - 1)(5 + 2)"; // INPUT EXPRESSION
    int count;
    ...
    if (tokens) {
        ...
        int new_count;
        char** fixed = insert_implicit_multiplication(tokens, count, &new_count);
        ...
        // Simple Shunting Yard Algorithm (Converts to RPN)
        ...
        for (int i = 0; i <     new_count; i++) {
            ...
            if (is_number(token)) {
                ...
                while (operator_stack_size > 0 && precedence(operator_stack[operator_stack_size - 1][0]) >= precedence(token[0])) {
                    rpn_tokens[rpn_len++] = operator_stack[--operator_stack_size];
                }
                operator_stack[operator_stack_size++] = token;
            }
        ...
        double result = evaluate_rpn(rpn_tokens, rpn_len);
